...
У вас должен быть ОДИН РЕПОЗИТОРИЙ на ВСЕ ДОМАШКИ. В репозитории для каждой домашки должен быть создан пакет (папка) с именем home_work_n
где n это порядковый номер домашнего задания.
в корне папки home_work_n должен находиться файла README в которы написано какой класс какой пункт ДЗ решает
...
2. Изучить приоритетность операторов по таблице. Сначала посчитать результаты всех ниже приведённых примеров устно, а потом
...
5. Разбираемся с операторами ветвления. Используя статью https://metanit.com/java/tutorial/2.9.php запросить у пользователя
его имя. В данном задании у вас должно получиться 3 класса.
...
7.** Создать СТАТИЧЕСКИЙ метод String toBinaryString(byte number) и возвращает двоичное представление числа. В методе можно
использовать обычную математику которая описана в теории. Теория https://planetcalc.ru/747. Если число отрицательное то выдавать дополнительный код. Пример:
...
8. Переделать задание 5.
	...
	8.2 Классы созданные ранее должны реализовывать интерфейс ICommunicationPrinter выполняя поставленную задачу в задании
	 5 и более не иметь точки входна внутри себя.
